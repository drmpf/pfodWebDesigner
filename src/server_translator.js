...
function translateRawText(rawTextString) {
    ...
    const xOffset = parts.length > 2 && parts[2] !== '' ? parseFloat(parts[2]) : undefined;
    const yOffset = parts.length > 3 && parts[3] !== '' ? parseFloat(parts[3]) : undefined;
    const alignment = parts.length > 4 && parts[4] !== '' ? parts[4] : undefined;
    ...
    if (alignment) {
        ...
        textObject.align = alignMap[alignment] || 'left';
    }
...
function translateRawValue(rawValueString) {
    ...
    const alignment = decimalsAndAlign.length > 1 ? decimalsAndAlign[1] : undefined;
    ...
    if (alignment) {
        const alignMap = { 'L': 'left', 'C': 'center', 'R': 'right' };
        valueObject.align = alignMap[alignment] || 'left';
    }
...
function translateRawZero(rawZeroString) {
    ...
    if (content === '') {
        ...
        if (parts.length !== 3) {
            throw new Error('Invalid pushZero format: must have exactly 3 parameters (col, row, scaling)');
        }
        
        const x = parseFloat(parts[0]); // col
        const y = parseFloat(parts[1]); // row
        const scale = parseFloat(parts[2]); // scaling
...
function translateRawTouchAction(rawTouchActionString) {
    ...
    try {
        ...
        if (primitiveRaw.includes('`')) {
            // Remove idx specification for touchAction primitives
            const parts = primitiveRaw.split('~');
            if (parts[0].includes('`')) {
                const typePrefix = parts[0].substring(0, parts[0].indexOf('`'));
                parts[0] = typePrefix;
                cleanPrimitiveRaw = parts.join('~');
            }
        }
        
        drawingPrimitive = translateRawItem(cleanPrimitiveRaw);
        
        // Remove idx from the result if it exists (touchAction primitives shouldn't have idx)
        if (drawingPrimitive.hasOwnProperty('idx')) {
            delete drawingPrimitive.idx;
        }
    ...
    rawData.raw_items.forEach((rawItem, index) => {
        if (rawItem && rawItem.trim() !== '') { // Ignore empty items
            try {
                ...
                console.error(`Error translating raw_items at line ${index + 1}: "${rawItem}":`, itemError.message);
                throw new Error(`Translation failed at line ${index + 1}: ${itemError.message}`);
            }
        }
    });
...
module.exports = { translateRawLine, translateRawRectangle, translateRawCircle, translateRawArc, translateRawText, translateRawValue, translateRawHide, translateRawUnhide, translateRawZero, translateRawTouchZone, translateRawErase, translateRawIndex, translateRawInsertDwg, translateRawTouchActionInput, translateRawTouchAction, translateRawItem, translateRawItemsToItemArray };